	 ...
	 $newnode->setAttribute("machines", $row['number_of_machines']);   
	  $newnode->setAttribute("rails", $row['rails']);   
	   $newnode->setAttribute("notes", $row['short_notes']);   
		...
		if ($retval3[$keyval]['email'] == $row['email']) { 		$newnode->setAttribute("emailed", 'success');

		 } elseif ($retval['email'] == $row['email']) { 		$newnode->setAttribute("emailed", 'un-opened');
	
		 } elseif ($retval1['email'] == $row['email']) {		$newnode->setAttribute("emailed", 'Hard Bounce');
	
		 } elseif ($retval2['email'] == $row['email']) { 		$newnode->setAttribute("emailed", 'Soft Bounce');
	
		 }
		 
		 
		 } else { 												$newnode->setAttribute("emailed", 'No Data');
