	...
	public function	form()
		{
			...
			{
				case 'both':
					$htmlForm .='<div class="essence">'. $ACL_LANG['USERNAME'].'/'.$ACL_LANG['EMAIL'];
					break;'</div>';
				case 'email':
					$htmlForm .='<div class="essence">'. $ACL_LANG['EMAIL'];
					break;'</div>';
				default:
					$htmlForm .='<div class="essence">'. $ACL_LANG['USERNAME'];
					break;'</div>';
			}						
			$htmlForm .= ':</label>'.
						 '<input type="text" name="u" id="u" class="textfield" />'.
						 '<label id="laid">'.$ACL_LANG['PASSWORD'].'</label>'.
					...
					if($u&&$p)
						{
							...
							$sql = "SELECT * FROM ".$_COOKIE['dbname']." WHERE ";
							switch(LOGIN_METHOD)
								{
									case 'both':
									...
									case 'email':
										...
										$sqlR .= "useremail='".mysql_real_escape_string($u)."'";
										break;
									default:
