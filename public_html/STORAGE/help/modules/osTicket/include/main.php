...
<form action="<?php echo($GLOBALS['conf']['modules']['osTicket']['selfurl']); ?>" method="POST">
<tr><td>
...
while ($tickets_row = mysql_fetch_array($tickets_res)) {
    ...
    if ($total >= $start and $total <= ($start + $config[tickets_per_page]) - 1 and $eval->status == strtolower($status) and ($_SESSION[user][type] == "client" or (@in_array($eval->cat, $oslogin[cat_access]) or $oslogin[cat_access][0] == "all"))) {
        ...
        <td align="center"><a href="<?php echo($GLOBALS['conf']['modules']['osTicket']['url']); ?><?=$page?>&a=view&id=<?=$eval->id?>"><?=$eval->id?></a></td>
        <td align="center"><?=$eval->short_time?></td>
        <td>&nbsp;<a href="<?php echo($GLOBALS['conf']['modules']['osTicket']['url']); ?><?=$page?>&a=view&id=<?=$eval->id?>"><?=$eval->subject?></a></td>
        <td>&nbsp;<?=$cat_row[name]?></td>
...
?>
    ...
    if ($_SESSION[user][type] == "client" or (@in_array($um[$i]->cat, $oslogin[cat_access]) or $oslogin[cat_access][0] == "all")) {
        ...
        ?>
            ...
            <td align="center"><a href="<?php echo($GLOBALS['conf']['modules']['osTicket']['selfurl']); ?>&a=view&id=<?=$um[$i]->id?>"><?=$um[$i]->id?></a></td>
            <td align="center"><?=$um[$i]->short_time?></td>
            <td>&nbsp;<a href="<?php echo($GLOBALS['conf']['modules']['osTicket']['selfurl']); ?>&a=view&id=<?=$um[$i]->id?>"><?=$um[$i]->subject?></a></td>
            <td>&nbsp;<?=$cat_row[name]?></td>
            <td align="center" <?=$color?>><?=$pri?></td>
            <td>&nbsp;<?=$um[$i]->name?></td>
...
?>
<input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/delete.gif" name="delete" value="Delete">
<?
}
switch ($status) {
    case "Closed":
    ?>
    <input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/reopen.gif" name="reopen" value="Reopen"> <input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/view_open.gif" name="view_open" value="View Open"> <input input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/view_answered.gif" name="view_answered" value="View Answered">
    <?
    break;
    case "Open":
    ?>
    <input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/close.gif" name="close" value="Close"> <input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/view_answered.gif" name="view_answered" value="View Answered"> <input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/view_closed.gif" name="view_closed" value="View Closed">
    <?
    break;
    case "Answered":
    ?>
    <input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/close.gif" name="close" value="Close"> <input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/view_open.gif" name="view_open" value="View Open"> <input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/view_closed.gif" name="view_closed" value="View Closed">
    <?
    break;
}
?>
<input type="image" src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/buttons/refresh.gif" name="refresh" value="Refresh">
</center>
</td></tr>
</form>
</table>
