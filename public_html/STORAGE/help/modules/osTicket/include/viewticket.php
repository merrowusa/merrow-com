...
$id=_addslashes($_REQUEST[id]);
$res=mysql_query("SELECT * FROM tickets WHERE ID='$_REQUEST[id]'");
...
$admin_permis = ($_SESSION[user][type] == "admin" and (@in_array($cat_row[ID], $oslogin[cat_access]) 
or $oslogin[cat_access][0] == "all" or $oslogin[ID] == ADMIN));
...
?>
    ...
    <form action="<?php echo($GLOBALS['conf']['modules']['osTicket']['url']); ?><?=$page?>" method="POST">
	<tr>
		<td>
	...
	<table cellpadding="0" cellspacing="0" border="0"><tr><td><img src="<?php echo($GLOBALS['conf']['modules']['osTicket']['realurl']); ?>images/spacer.gif" width="1" height="5"></td></tr></table>
	...
	while ($msg_row = mysql_fetch_array($msg_res)) {
		...
		while ($file = mysql_fetch_array($files)) {
            ...
            if ($_SESSION[user][type] == "admin") {
		      $fstr .= "<a href='".$GLOBALS['conf']['modules']['osTicket']['url']."$config[attachment_url]?file=$file[filename]'>" . $filename . "</a> ($size kb); ";
		    }
		    else {
        ...
        if ($fstr or ($msg_row[headers] and $_SESSION[user][type] == "admin")) {
            ...
            if ($msg_row[headers]) {
                ?>
                 ( <a href="<?php echo($GLOBALS['conf']['modules']['osTicket']['url']); ?>admin.php&a=headers&msg=<?=$msg_row[ID]?>" target="_new">headers</a> )
                <?
            }
            ?>
            </td>
		...
		while ($answer_row = mysql_fetch_array($answers_res)) {
    		...
    		if ($files[filename]) {
                $fstr = "<a href='".$GLOBALS['conf']['modules']['osTicket']['url']."$config[attachment_url]?file=$files[filename]'>" . $filename . "</a> ($size kb)";
                ?>
                <tr class="msgAttachments">
...
<table align="center" cellspacing="0" cellpadding="3" width="100%" border=0>
  <tr> 
    <td align="center"><br>
      <form action="<?php echo($GLOBALS['conf']['modules']['osTicket']['selfurl']); ?>" method="post" enctype="multipart/form-data">
    	...
    	<table width="100%" cellpadding="3" cellspacing="0" border="0">
    		<tr>
    			<td align="center"><a href="<?php echo($GLOBALS['conf']['modules']['osTicket']['selfurl']); ?>">Back to main</a></td>
    		</tr>
    	</table>
    	<br>
    	<?
