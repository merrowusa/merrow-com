...
?>
    ...
    // $Id: live.php,v 1.4 2005/05/26 08:27:35 mikebird Exp $ 
...
{ this.time = Misc.epoch(); if (x == 'initiate') { if (this.initiate_flag !== 1) { if (document.getElementById('div_initiate') !== null) { document.getElementById('div_initiate').style.visibility = 'visible'; window.setTimeout('document.getElementById(\'div_initiate\').style.visibility = \'hidden\';', <?php echo($GLOBALS['conf']['initiate_timeout']); ?>); this.initiate_flag = 1;}
}
} else { if (y == '') { this.chatwin = window.open('<?php echo($GLOBALS['conf']['url']); ?>/live/main.php', 'guest_<?php echo($chatid); ?>', 'toolbar=no, scrollbars=yes, status=no, resizable=yes, location=no, menubar=no, directories=no, width=400, height=400');} else { this.chatwin = window.open('<?php echo($GLOBALS['conf']['url']); ?>/live/main.php?departmentid='+y, 'guest_<?php echo($chatid); ?>', 'toolbar=no, scrollbars=yes, status=no, resizable=yes, location=no, menubar=no, directories=no, width=400, height=400');}
if (this.chatwin.opener == null) { this.chatwin.opener = self;}
}
}
<?php
if (!isset($_GET["invisible"])) { if (isset($_GET["text"])) { ?>
if (this.status == 'online') { document.write("<a href=\"javascript:Live.launch('chat', '<?php echo($departmentid); ?>')\" onMouseOver=\"Misc.statusbar('<? echo($GLOBALS['lang']['click_for_live_help']); ?>');return true;\" onMouseOut=\"Misc.statusbar('');return true;\"><?php echo($GLOBALS['lang']['live_text_online']); ?></a>");} else { document.write("<a href=\"javascript:Live.launch('chat', '<?php echo($departmentid); ?>')\" onMouseOver=\"Misc.statusbar('<? echo($GLOBALS['lang']['click_for_live_help']); ?>');return true;\" onMouseOut=\"Misc.statusbar('');return true;\"><?php echo($GLOBALS['lang']['live_text_offline']); ?></a>");}
<?php } else { ?>
