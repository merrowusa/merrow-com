<?php
// File : french_canadian.tutorial.php3
// Translation by Guylhem Aznar <guylhem@metalab.unc.edu>
...
<HEAD>
<TITLE>Tutoriel en français</TITLE>
...
<P CLASS="redText">
Attention : les utilisateurs de Netscape doivent définir leur langue comme encodage par défaut sinon chacun des caractères des messages risque d'apparaître sous forme de points d'interrogation.<BR>
...
<A HREF="#modProfile" CLASS="topLink">Modifier<?php if (C_SHOW_DEL_PROF == "1") echo("/supprimer"); ?> son profil</A><BR>
<?php
if (C_VERSION == "2") 
{
	?>
...
{
	?>
	&nbsp&nbsp&nbsp&nbsp<A HREF="#smilies" CLASS="topLink">Trombines</A><BR>
	<?php
};
...
&nbsp&nbsp&nbsp&nbsp<A HREF="#kick" CLASS="topLink">Exclure un utilisateur</A><BR>
<?php
if (C_BANISH != "0")
{
	?>
...
?>
&nbsp&nbsp&nbsp&nbsp<A HREF="#promote" CLASS="topLink">Faire accéder un utilisateur au statut de modérateur</A><BR>
...
if (C_MULTI_LANG == "1") 
{
	...
	<P ALIGN="center">
	<IMG SRC="tutorials/flags.gif" HEIGHT="44" WIDTH="424" ALT="Drapeau de sélection de langue">
...
<P>
Si vous vous êtes déjà enregistré, il vous suffit d'inscrire votre nom d'utilisateur, c'est à dire votre pseudonyme, et votre mot de passe dans les champs idoines. Sélectionnez ensuite un salon de discussion et cliquez sur le bouton « Discuter... »<BR>
<?php
...
else
{
	?>
	<P>
	Sinon vous pouvez commencer par vous <A HREF="#register">enregistrer</A> ou simplement choisir un pseudonyme et un salon de discussion. Dans ce dernier cas votre pseudonyme ne sera pas réservé : d'autres utilisateurs pourront s'en servir une fois que vous vous serez déconnectés. Dans certains salons de discussions il est obligatoire de s'enregistrer.
	<?php
}
?>
<BR><P ALIGN="right"><A HREF="#top">Début de la page</A></P>
...
<P>	
<UL>
	<LI>Choisissez d'abord votre pseudonyme <?php if (!C_EMAIL_PASWD) echo(" et votre mot de passe"); ?>. Votre pseudonyme ou nom d'utilisateur est votre identifiant au sein des salons de discussions auxquels vous participerez. Il ne peut contenir ni caractère espace, ni virgule, ni barre oblique renversée (\).
<?php if (C_NO_SWEAR == "1") echo(" Il ne pourra pas non plus contenir de \"gros mots\"."); ?>
	<LI>Entrez ensuite vos nom, prénom et courriel. Sans ces informations l'enregistrement de votre profil vous sera refusé. L'indication du sexe est facultative.
	<LI>Si vous disposez d'un site personnel, vous pouvez en indiquer l'adresse.
	...
	<LI>Enfin, en cochant la case correspondante vous pouvez choisir de rendre votre adresse électronique visible pour les utilisateurs qui consulteront votre profil, qui pourront ainsi vous envoyer du courriel. Si vous préférez que cette information reste cachée, laissez la case vierge.
...
<P>
<A NAME="modProfile"></A>Bien évidemment, un utilisateur enregistré peut modifier<?php if (C_SHOW_DEL_PROF == "1") echo("/effacer"); ?> son profil en cliquant sur <?php echo((C_SHOW_DEL_PROF == "0"? "le lien correspondant" : "les liens correspondants")); ?> de la page d'accueil.<BR>
<BR><P ALIGN="right"><A HREF="#top">Début de la page</A></P>
<P>
<HR>
...
if (C_VERSION == "2") 
{
	...
	<P>
	Un utilisateur enregistré peut créer son propre salon de discussion. S'il le déclare de type privé, seul un utilisateur en connaissant le nom (il en aura été informé par ailleurs) pourra y accéder car ce nom n'apparaîtra nulle part, sauf aux utilisateurs qui discuteront dans ce même salon.<BR>
	<P>
	Le nom d'un salon ne peut contenir ni virgule, ni barre oblique renversée (\).<?php if (C_NO_SWEAR == "1") echo(" Il ne peut pas plus comprendre de \"gros mots\"."); ?>
	<BR><P ALIGN="right"><A HREF="#top">Début de la page</A></P>
	<P>
	<HR>
	<?php
...
<OL>
	...
	<LI>une petite icône est affichée devant le pseudonyme d'un utilisateur enregistré et définit son genre (cliquer sur cette icône activera la <A HREF="#whois">fenêtre affichant le profil</A> de cet utilisateur), alors que c'est une espace vide qui précède le pseudonyme des utilisateurs non-enregistrés&nbsp;;<BR>
	<LI>le pseudonyme de l'administrateur et ceux des modérateurs sont affichés en italique.
</OL>
<P><I>Par exemple</I>, de la capture d'écran ci-dessous on peut conclure que:
<TABLE BORDER=0 CELLSPACING=10>
<TR>
	<TD>
		<IMG SRC="images/tutorials/usersList.gif" WIDTH=128 HEIGHT=145 BORDER=0 ALT="Liste d'utilisateurs">
	</TD>
	<TD>
	<UL>
		<LI>Nicolas est l'administrateur ou bien l'un des modérateurs du salon&nbsp;;<BR><BR>
		<LI>alien (de genre inconnu), Jezek2 et Caridad sont des utilisateurs enregistrés sans pouvoirs particuliers dans le salon&nbsp;;<BR><BR>
...
<P>
<I>Par exemple :</I> /quit À bientôt !
<P>
...
<P>
En cliquant sur le lien affichant le nombre d'utilisateurs de la page d'accueil ou bien, une fois dans une salon, sur l'icône <IMG SRC="images/popup.gif" WIDTH=13 HEIGHT=13 BORDER=0 ALT="Liste des utilisateurs"> en haut et à droite de votre écran, vous ouvrirez une petite fenêtre indépendante présentant la liste des utilisateurs connectés et des salons dans lesquels ils sont présents en temps presque réél.<BR>
...
<UL>
	...
	{
		?>
		<LI>L'effacement permet de réinitialiser le cadre des messages en ne conservant que les 5 derniers d'entre eux.<BR>Tapez "/clear" sans les guillemets.
		<P>
		<?php
	}
	else
	{
		?>
		<LI>Le rangement inverse l'ordre d'affichage des messages (des plus récents en haut aux plus récents en bas et vice versa).<BR>Tapez "/order" sans les guillemets.
		<P>
		<?php
	};
	?>
	<LI>La notification vous permet de choisir s'il faut afficher ou pas les notifications concernant l'entrée et la sortie des utilisateurs dans le salon courant. Par défaut cette option est <?php echo(C_NOTIFY ? "activée" : "désactivée"); ?> et les notifications <?php echo(C_NOTIFY ? "sont" : "ne sont pas"); ?> affichées.<BR>Tapez "/notify" sans les guillemets.
	<P>
	<LI>Le datage active ou supprime l'affichage de l'heure à laquelle les messages ont été postés ainsi que l'heure serveur dans la barre des tâches. Par défaut cette option est <?php echo(C_SHOW_TIMESTAMP ? "activée" : "désactivée"); ?>.<BR>Tapez "/timestamp" sans les guillemets.
	<P>
	<LI>Le délai de rafraîchissement du cadre contenant les messages peut être modifié. La valeur par défaut est de <?php echo(C_MSG_REFRESH); ?> secondes. Pour la modifier, tapez "/refresh n" sans les guillemets où n est le délai qui vous convient exprimé en secondes.
	<P>
	...
	{
		?>
		<LI>Le nombre de messages à afficher dans la fenêtre principale peut être défini par une valeur n. Pour modifier cette valeur, tapez "/show n" sans les guillemets où n correspond au nombre des messages désirés.
		<P>
	...
	{
		?>
		<LI>Ces commandes provoquent une réinitialisation de l'écran et l'affichage d'un nombre défini de messages parmi les derniers reçus. Tapez "/show n" ou "/last n" sans les guillemets où n correspond au nombre des messages à afficher.
		<P>
...
<P>
Une fois entré dans un salon de discussion, vous pourrez obtenir de l'aide en cliquant sur l'image <IMG SRC="images/helpOff.gif" WIDTH=15 HEIGHT=15 BORDER=0 ALT="?"> qui est placée à gauche de la « boite à messages ». Vous pourrez encore utiliser les <B>commandes "/help" ou "/?"</B> à cet effet.
<BR><P ALIGN="right"><A HREF="#top">Début de la page</A></P>
<P>
<P>
<HR>
...
if (C_USE_SMILIES == "1")
{
	...
	unset($SmiliesTbl);
	?>
	<FONT SIZE="+1"><A NAME="smilies"><B>Trombines:</B></A></FONT>
	<P>Vos messages peuvent être agrémentés d'éléments graphiques présentant votre état émotionnel. Vous trouverez ci-après les codes qu'il faut utiliser dans vos messages pour obtenir chacun de ces émoticons lors de l'affichage dans le cadre dédié.
	<P ALIGN="center">
...
if (C_HTML_TAGS_KEEP != "none")
{
	...
	<P>
	Pour obtenir des hyperliens vers des sites internet ou des adresses électroniques, tapez simplement l'adresse en question dans votre message sans aucune balise. Elle sera automatiquement traitée.
	<BR><P ALIGN="right"><A HREF="#top">Début de la page</A></P>
	<P>
	<P>
	<HR>
...
<P>
Il est possible d'afficher une petite fenêtre dans laquelle vous pourrez modifier votre profil de façon limitée : votre pseudonyme et votre mot de passe ne pourront en aucun cas être changés (utilisez le lien sur la page d'accueil pour ce faire).<BR>Tapez "/profile" sans guillemets.
<BR><P ALIGN="right"><A HREF="#top">Début de la page</A></P>
<P>
<HR>
...
<P>
La commande « ! » insère dans la « boite à messages » le dernier texte qui y a été entré, qu'il s'agisse d'une commande ou d'un message.<BR>Tapez "/!" sans guillemets.
<BR><P ALIGN="right"><A HREF="#top">Début de la page</A></P>
<P>
<HR>
...
<P>
Cliquer une fois sur le nom d'un utilisateur apparaissant dans la liste située à droite de votre écran insèrera son nom dans la « boite à messages ». Ceci permet d'indiquer que le message que vous envoyez s'adresse particulièrement à lui, par example.
<BR><P ALIGN="right"><A HREF="#top">Début de la page</A></P>
<P>
<HR>
...
<P>
Pour faire parvenir un message privé à un utilisateur présent dans le même salon que vous, tapez la <B>commande "/msg pseudonyme texte_du_message" ou "/to pseudonyme texte_du_message"</B> sans les guillemets.
<P>
...
<P>
Pour ne pas voir s'afficher les messages provenant d'un utilisateur particulier, utilisez la commande "/ignore pseudo" sans guillemets.
<P>
...
<P>
Pour visualiser le profil d'un autre utilisateur, tapez la <B>commande "/whois pseudonyme"</B> sans guillemets.
<P>
...
<P>
<I>Par exemple :</I> /announce Mise à jour du site à 20h00 ce soir, risque de perturbations.</I>
<BR><P ALIGN="right"><A HREF="#top">Début de la page</A></P>
<P>
<HR>

<P>
<FONT SIZE="+1"><A NAME="kick"><B>Exclure un utilisateur :</B></A></FONT>
<P>
...
<P>
<I>Par exemple, si Coco est le pseudonyme de l'utilisateur à bannir :</I> /kick Coco</I>
<BR><P ALIGN="right"><A HREF="#top">Début de la page</A></P>
<P>
<HR>
...
<P>
<I>Par exemple, si Coco est le pseudonyme de l'utilisateur à "promouvoir" :</I> /promote Coco</I>
<P>
