...
session_name('SLOGIC');
if (isset($_COOKIE['staff_sid']))
{
	session_id($_COOKIE['staff_sid']);
...
else if (!isset($_COOKIE['staff_sid']))
{
...
if ((isset($_COOKIE['staff_sid'])) && ((!isset($_SESSION['login_verified'])) || ($_SESSION['browser_type'] != $_SERVER['HTTP_USER_AGENT'])))
{
	// the user is unknown, so reset the cookie (can result, when the session gets deleted from the server)
	setcookie ('staff_sid', '', time()-48*3600);
}
...
{
	// verify the login
	if (login_verify($_POST['login_user'], $_POST['login_pass'], 'staff'))
	{
		$db = new db_module(DB_HOST,DB_USER,DB_PASS,DB_DATABASE);
		if ($db->db_connected)
		{
			// get the user selected language from his profile
			$db->db_query('SELECT language FROM staff
...
if ((isset($_SESSION['login_verified'])) && ($_SESSION['browser_type'] == $_SERVER['HTTP_USER_AGENT']) && ($_SESSION['table'] == 'staff'))
{
	...
	if ((!isset($_GET['tpl'])) && (!isset($_POST['tpl'])))
	{
		...
		$_POST['tpl'] = 'slogic_main';
	 	$user_screen = join ('', file (SCRIPT_PATH.'modules/staff/slogic/templates/pages/slogic_main.tpl'));
	...
	else 
	{
		...
		{
			if (file_exists(SCRIPT_PATH.'modules/staff/'.$template.'/templates/pages/'.$tpl.'.tpl'))
			{
				$user_screen = join ('', file (SCRIPT_PATH.'modules/staff/'.$template.'/templates/pages/'.$tpl.'.tpl'));
			}
			else if (DEBUG_MODE == 'on')
			{
				echo show_debug_message($tpl.'.tpl','file doesn\'t exist!'.SCRIPT_PATH.'modules/staff/'.$template.'/templates/pages/'.$tpl.'.tpl');
			}
		}
		else if (DEBUG_MODE == 'on')
		{
	...
	{
		$get_header = join ('', file (SCRIPT_PATH.'modules/staff/slogic/templates/functions/slogic_header.tpl'));
		$get_footer = join ('', file (SCRIPT_PATH.'modules/staff/slogic/templates/functions/slogic_footer.tpl'));
	...
	for ($i = 1; $i != sizeof($get_functions); $i++)
	{
		...
		if ((in_array($execute_function,$executable_functions)) 
			&& (is_file(SCRIPT_PATH.'modules/staff/'.$get_module_name.'/functions/'.$execute_function.'.php'))
			...
			)
		{
			...
			{
				require SCRIPT_PATH.'modules/staff/'.$get_module_name.'/functions/'.$execute_function.'.php';
			}
		...
		{
			if (!file_exists(SCRIPT_PATH.'modules/staff/'.$get_module_name.'/'))
			{
				$user_screen = str_replace('{'.$get_function[0].'}',
											show_debug_message('{'.$get_function[0].'}',' - the needed module doesn\'t exist!'),
											$user_screen);
			}
			else if (!file_exists(SCRIPT_PATH.'modules/staff/'.$get_module_name.'/functions/'.$execute_function.'.php'))
			{
				$user_screen = str_replace('{'.$get_function[0].'}',
...
else 
{
	...
	$user_screen = join ('', file (SCRIPT_PATH.'modules/staff/slogic/templates/pages/slogic_login.tpl'));
	...
	if ($template_system == 'header_footer')
	{
		...
		// replace the "{LANG_...}" placeholders in $user_screen with the right entry from the $language array
		require (SCRIPT_PATH.'modules/staff/slogic/languages/'.DEFAULT_LANGUAGE.'/slogic_language.inc');
