...
// delete note(s) as provided by $_POST['del'] array
function tickets_delete_note()
{
	...
	if ($db->db_connected)
	{
		$this->query_result = $db->db_query('DELETE FROM notes WHERE id="'.$make_query.'"');
	}
	$db->db_close();
	...
	$user_screen = str_replace('{MESSAGE}','{LANG_TICKETS_LIST_NOTES_DELETED}',$user_screen);
