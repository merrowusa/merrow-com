...
// delete ticket(s) as provided by $_POST['del'] array
function tickets_delete_ticket()
{
	GLOBAL $user_screen;
	...
	if ($db->db_connected)
	{
		$this->query_result = $db->db_query('DELETE FROM tickets WHERE id="'.$make_query.'"');
	}
	$db->db_close();
	...
	$user_screen = str_replace('{MESSAGE}','{LANG_TICKETS_LIST_TICKETS_DELETED}',$user_screen);
