...
// display edit user form & add user to database
function slogic_edit_user()
{
	GLOBAL $user_screen;
	
	$output = join ('', file (SCRIPT_PATH.'modules/admin/slogic/templates/functions/slogic_edit_user.tpl'));
	
	$output = str_replace('{SELF}',$_SERVER['PHP_SELF'].'?tpl=slogic_edit_user&lang='.LANGUAGE.'&id='.$_GET['id'],$output);
	...
	if (isset($_POST['username']))
	{
		   ...
		   )
		{
			...
			$user_screen = str_replace('{MESSAGE}','{LANG_SLOGIC_USER_EDITED_INVALID}',$user_screen);
		}
		else 
		{	
			$make_query = array();
			...
			{
				// update user
				$this->query_result = $db->db_query('UPDATE users SET '.implode(',',$make_query).' WHERE id="'.$_GET['id'].'"');
			}
			$db->db_close();
			...
			$user_screen = str_replace('{SECONDS}','3',$user_screen);
			$user_screen = str_replace('{URL}',$_SERVER['PHP_SELF'].'?tpl=slogic_list_users&lang='.LANGUAGE,$user_screen);
			$user_screen = str_replace('{MESSAGE}','{LANG_SLOGIC_USER_EDITED}',$user_screen);
		}
	}
	else 
	{
		...
		{
			// get user info
			$this->query_result = $db->db_query('SELECT * FROM users WHERE id="'.$_GET['id'].'"');
