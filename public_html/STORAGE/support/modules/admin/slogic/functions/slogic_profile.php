...
function slogic_profile()
{
	...
	if (isset($_POST['email']))
	{
		...
		if (($_POST['password'] != $_POST['password2']) && ($_POST['password'] != ''))
		{
			...
			$user_screen = str_replace('{URL}',$_SERVER['PHP_SELF'].'?tpl=slogic_profile&lang='.LANGUAGE,$user_screen);
			$user_screen = str_replace('{MESSAGE}','{LANG_SLOGIC_PASSWORD_MISMATCH}',$user_screen);
		}
		else 
		{
			// change requested -> make changes & logout/redirect
			$db = new db_module(DB_HOST,DB_USER,DB_PASS,DB_DATABASE);
			if ($db->db_connected)
			{
				// get the category IDs and the subscribers to check them with the $_POST['category'] array
				$this->query_result = $db->db_query('UPDATE admin SET '.implode(',',$make_query).' WHERE username="'.$_SESSION['login_user'].'"');
				$db->db_close();
			}
			$user_screen = join ('', file (SCRIPT_PATH.'modules/admin/slogic/templates/pages/slogic_redirect.tpl'));
			$user_screen = str_replace('{SECONDS}','3',$user_screen);
	...
	{
		$output = join ('', file (SCRIPT_PATH.'modules/admin/slogic/templates/functions/slogic_profile.tpl'));
		...
		if ($db->db_connected)
		{
			...
			$this->query_result = $db->db_query('SELECT '.implode(',',$placeholders).' FROM admin WHERE username="'.$_SESSION['login_user'].'"');
