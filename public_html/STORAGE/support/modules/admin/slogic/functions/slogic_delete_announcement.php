<?php
/***************************************************************************
								slogic_delete_announcement.php
							--------------------------------------
			created				: 13.03.2003
			edited				: -
...
// delete announcements as provided by $_POST['del'] array
function slogic_delete_announcement()
{
	GLOBAL $user_screen;
	...
	if ($db->db_connected)
	{
		...
		$this->query_result = $db->db_query('DELETE FROM announcements WHERE id="'.$make_query.'"');
	}
	$db->db_close();
	...
	$user_screen = str_replace('{MESSAGE}','{LANG_SLOGIC_ANNOUNCEMENTS_DELETED}',$user_screen);
