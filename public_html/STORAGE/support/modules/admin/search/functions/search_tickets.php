...
function search_tickets()
{
	...
	$output = join ('', file (SCRIPT_PATH.'modules/admin/search/templates/functions/search_tickets.tpl'));
	...
	if (isset($_POST['search']))
	{
		if (trim($_POST['search']) == '')
		{
			...
			$user_screen = str_replace('{URL}',$_SERVER['PHP_SELF'].'?tpl=search_tickets&lang='.LANGUAGE,$user_screen);
			$user_screen = str_replace('{MESSAGE}','{LANG_WRONG_SEARCH_REDIRECT}',$user_screen);
		}
		else 
		{
			// generate search URL and redirect to it
	...
	else if (isset($_GET['search']))
	{
		...
		if ($db->db_connected)
		{
			if (isset($_GET['search_in_tickets']))
			{
				...
				$tickets_output = join ('', file (SCRIPT_PATH.'modules/admin/search/templates/functions/search_results_tickets.tpl'));
			...
			{
				// show notes results
				$notes_output = join ('', file (SCRIPT_PATH.'modules/admin/search/templates/functions/search_results_notes.tpl'));
