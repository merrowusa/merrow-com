		...
		if ($numrows != 0)
		{
			...
			if ($numrows != 0)
			{
				...
				if (($row['staff'] == $_SESSION['login_user']) && ($row['status'] == 'assigned'))
				{
					...
					$user_screen = str_replace('{MESSAGE}','{LANG_TICKETS_MOVE_MOVED} '.$_POST['staff'].'.',$user_screen);

					// send the staff member an email, if set in his profile
					$this->query_result = $db->db_query('SELECT email,get_email FROM staff WHERE username="'.$_POST['staff'].'"');
					$row = $db->db_fetch_array($this->query_result);
					if ($row['get_email'] == 'yes')
					{
						...
						$this->query_result = $db->db_query('SELECT subject FROM config WHERE name="toggle_staff_move_ticket"');
						$row8 = $db->db_fetch_array($this->query_result);
						$send_out_email = $row8['subject'];

						$this->query_result = $db->db_query('SELECT subject,content FROM config WHERE name="staff_move_ticket"');
					...
					$this->query_result = $db->db_query('UPDATE tickets SET staff=\''.$_POST['staff'].'\' WHERE id=\''.$_GET['id'].'\'');					
				...
				else if ($row['status'] == 'open')
				{
					...
					$user_screen = str_replace('{SECONDS}','3',$user_screen);
					$user_screen = str_replace('{URL}',$_SERVER['PHP_SELF'].'?tpl=tickets_show_ticket&lang='.LANGUAGE.'&id='.$_GET['id'],$user_screen);
					$user_screen = str_replace('{MESSAGE}','{LANG_TICKETS_MOVE_OPEN}',$user_screen);
				}
				else if ($row['status'] == 'closed')
				{
					...
					$user_screen = str_replace('{SECONDS}','3',$user_screen);
					$user_screen = str_replace('{URL}',$_SERVER['PHP_SELF'].'?tpl=tickets_show_ticket&lang='.LANGUAGE.'&id='.$_GET['id'],$user_screen);
					$user_screen = str_replace('{MESSAGE}','{LANG_TICKETS_MOVE_CLOSED}',$user_screen);
				}
				else 
				{
					$user_screen = join ('', file (SCRIPT_PATH.'modules/staff/slogic/templates/pages/slogic_redirect.tpl'));
					...
					$user_screen = str_replace('{MESSAGE}','{LANG_TICKETS_MOVE_INVALID}',$user_screen);
				}
			}
			else 
			{
