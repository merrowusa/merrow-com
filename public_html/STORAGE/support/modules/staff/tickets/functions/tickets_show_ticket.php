...
function tickets_show_ticket()
{
	...
	$output = join ('', file (SCRIPT_PATH.'modules/staff/tickets/templates/functions/tickets_show_ticket.tpl'));

	$output = str_replace('{USER_TICKET_HISTORY}','<a href="'.$_SERVER['PHP_SELF'].'?tpl=tickets_show_history&user={USER}&lang='.LANGUAGE.'">{LANG_TICKETS_HISTORY_ICON}</a>',$output);
	$output = str_replace('{SHOW_USER_PROFILE}','<a href="'.$_SERVER['PHP_SELF'].'?tpl=slogic_show_user_profile&user={USER}&lang='.LANGUAGE.'">{LANG_TICKETS_PROFILE_ICON}</a>',$output);
	...
	$this->query_result = $db->db_query('SELECT * FROM attachments WHERE ticket_id=\''.$_GET['id'].'\'');
	$numrows = $db->db_num_rows($this->query_result);
	if ($numrows != 0)
	{
		$tmp_attachments = '';
		while ($row = $db->db_fetch_array($this->query_result))
		{
			$tmp_attachments .= '<a href="'.$_SERVER['PHP_SELF'].'?tpl=tickets_confirm_delete_attachment&id='.$_GET['id'].'&attachment_id='.$row['id'].'&lang='.LANGUAGE.'">{LANG_TICKETS_TRASH_ICON}</a> - <a href="'.$_SERVER['PHP_SELF'].'?tpl=tickets_show_attachment&id='.$row['id'].'" target="_blank">{LANG_TICKETS_ATTACHMENT_ICON} '.$row['filename'].'</a><br>';
		}
