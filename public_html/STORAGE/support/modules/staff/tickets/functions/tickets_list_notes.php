...
function tickets_list_notes()
{
	...
	$output = join ('', file (SCRIPT_PATH.'modules/staff/tickets/templates/functions/tickets_list_notes.tpl'));
	...
	if ($db->db_connected)
	{
		$this->query_result = $db->db_query('SELECT * FROM notes WHERE ticket_id=\''.$_GET['id'].'\' ORDER BY sent_date');
			...
			while ($row = $db->db_fetch_array($this->query_result))
			{
				...
				foreach ($row as $key => $value) 
				{
					if (in_array($key,$list_placeholders))
					{
						...
						{
							if (($value == 'staff') && ($row['show_to'] == 'staff'))
							{
								$tmp_output[$i] = str_replace('{'.strtoupper($key).'}','<img src="'.SCRIPT_URL.'/modules/staff/tickets/languages/'.LANGUAGE.'/staff_2_staff.gif">',$tmp_output[$i]);
							}
							else 
							{
								$tmp_output[$i] = str_replace('{'.strtoupper($key).'}','<img src="'.SCRIPT_URL.'/modules/staff/tickets/languages/'.LANGUAGE.'/'.$value.'.gif">',$tmp_output[$i]);
							}
						}
						else 
						{
							$tmp_output[$i] = str_replace('{'.strtoupper($key).'}',$value,$tmp_output[$i]);
						}
					}
					if (($row['sender'] == $_SESSION['login_user']) && ($row['who_is'] != 'user') && ($status != 'closed'))
					{
						$tmp_output[$i] = str_replace('{EDIT}','<a href="'.$_SERVER['PHP_SELF'].'?tpl=tickets_show_ticket_edit&lang='.LANGUAGE.'&id='.$_GET['id'].'&note_id='.$row['id'].'">{LANG_TICKETS_EDIT_ICON}</a>',$tmp_output[$i]);
						$tmp_output[$i] = str_replace('{DELETE}','<a href="'.$_SERVER['PHP_SELF'].'?tpl=tickets_confirm_delete_note&lang='.LANGUAGE.'&id='.$_GET['id'].'&note_id='.$row['id'].'">{LANG_TICKETS_DELETE_ICON}</a>',$tmp_output[$i]);
					}
					else 
					{
