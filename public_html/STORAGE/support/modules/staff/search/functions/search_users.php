...
function search_users()
{
	...
	$output = join ('', file (SCRIPT_PATH.'modules/staff/search/templates/functions/search_users.tpl'));
	...
	else if (isset($_GET['search']))
	{
		...
		{
			// show user results
			$users_output = join ('', file (SCRIPT_PATH.'modules/staff/search/templates/functions/search_results_users.tpl'));
			...
			if ($numrows != 0)
			{
				...
				while ($row = $db->db_fetch_array($this->query_result))
				{
					...
					$users_results[$i] = str_replace('{USERNAME}','<a href="mailto:'.$row['email'].'">'.$row['username'].'</a>',$users_results[$i]);
					$users_results[$i] = str_replace('{EMAIL}',$row['email'],$users_results[$i]);
