...
<form name="limit_form" method="get" action="{SELF}">
  <tr> 
    ...
    <td height="20" colspan="3" class="head" align="right">Show next 20 replies starting at row: <input type="text" class="input"  name="limit" size="4"><input type="hidden" name="tpl" value="slogic_list_quick_replies"><input type="hidden" name="lang" value="{LANG}"><input type="hidden" name="order_table" value="quick_replies"> <input class="button" type="submit" name="submit" value="Go!"></td>
  </tr>
  <tr> 
...
<form name="quick_replies_form" method="post" action="{SELF}">
  <tr> 
    ...
    <td height="17" width="30%"><b>{LANG_SLOGIC_MESSAGE}</b>&nbsp;{ORDER_MESSAGE}{LANG_TICKETS_ARROW_DOWN}</a>&nbsp;{ORDER_MESSAGE_DESC}{LANG_TICKETS_ARROW_UP}</a></td>
    <td height="17" align="center" width="10%"><b>{LANG_SLOGIC_EDIT}</b></td>
    <td height="17" align="center" width="20%">{DELETE_BUTTON} <input type="checkbox" class="checkbox" name="delete_all" title="{LANG_SLOGIC_LIST_QUICK_REPLIES_DELETE_ALL}" onclick="toggle_all(this)">({LANG_SLOGIC_ALL})</td>
  </tr>
<QUICK_REPLIES>
  <tr class="subhead">
