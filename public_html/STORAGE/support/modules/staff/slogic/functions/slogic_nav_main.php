...
{
	// we need the navigation from the staff.php file, so make it global
	...
	for ($i=0; $i != sizeof($module); $i++)
	{
		...
		$navigation_template = join ('', file (SCRIPT_PATH.'modules/staff/slogic/templates/functions/slogic_nav_main.tpl'));
		...
		for ($j=0; $j != sizeof($get_navigation); $j++)
		{
			...
			if (substr($get_navigation[$j], 0, 13) == 'tickets_list_')
			{
				...
				{
					require SCRIPT_PATH.'modules/staff/tickets/functions/'.$tmp_function.'.php';
					$tmp_count = $tmp_function();
				}
				$tmp_count = ' - '.$tmp_count;
			}
		...
		$make_navigation[$i] = join ('', file (SCRIPT_PATH.'modules/staff/slogic/templates/functions/slogic_nav_module.tpl'));
