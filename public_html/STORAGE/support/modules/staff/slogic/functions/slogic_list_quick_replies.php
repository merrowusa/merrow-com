...
function slogic_list_quick_replies()
{
	...
	{
		include SCRIPT_PATH.'modules/staff/slogic/functions/slogic_count_quick_replies.php';
	}
	$limit_max = slogic_count_quick_replies();
	$get_max_limit = $limit_max;
	$limit_max = ceil($limit_max / 20);
	
	$output = join ('', file (SCRIPT_PATH.'modules/staff/slogic/templates/functions/slogic_list_quick_replies.tpl'));
	...
	{
		// get quick replies
		$this->query_result = $db->db_query('SELECT * FROM quick_replies WHERE username="'.$_SESSION['login_user'].'" ORDER BY '.$order.' LIMIT '.$limit.',20');
		...
		if ($numrows != 0)
		{
			...
			while ($row = $db->db_fetch_array($this->query_result))
			{
				...
				$make_quick_reply[$i] = str_replace('{DELETE}','<input type="checkbox" class="checkbox" name="del[]" value="'.$row['id'].'" onclick="toggle(this);" title="{LANG_SLOGIC_DELETE}">',$make_quick_reply[$i]);
				$i++;
			}
