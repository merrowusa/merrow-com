...
{
	$output = join ('', file (SCRIPT_PATH.'modules/staff/slogic/templates/functions/slogic_select_language.tpl'));
	
	$language_part = explode('<LANGUAGE>',$output);
	$language_part = explode('</LANGUAGE>',$language_part[1]);
	$language_part = $language_part[0];
	
	if ($handle = @opendir(SCRIPT_PATH.'modules/staff/slogic/languages/'))
	{
		$i=0;
		$get_languages = array();
		while (false !== ($file = @readdir($handle)))
		{
			if (($file != '.') && ($file != '..') && (!is_file(SCRIPT_PATH.'modules/staff/slogic/languages/'.$file))) 
			{
