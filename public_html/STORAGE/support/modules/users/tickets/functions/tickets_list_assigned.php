...
// list all user assigned tickets
function tickets_list_assigned()
{
	...
	{
		include SCRIPT_PATH.'modules/users/tickets/functions/tickets_count_assigned_tickets.php';
	}
	...
	$output = join ('', file (SCRIPT_PATH.'modules/users/tickets/templates/functions/tickets_list_assigned.tpl'));
	...
	$query = 'SELECT * FROM tickets WHERE user="'.$_SESSION['login_user'].'" AND status="assigned" ORDER BY '.$order.' LIMIT '.$limit.',20';
