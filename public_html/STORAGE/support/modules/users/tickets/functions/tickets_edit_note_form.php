...
function tickets_edit_note_form()
{
	...
	$output = join ('', file (SCRIPT_PATH.'modules/users/tickets/templates/functions/tickets_edit_note_form.tpl'));
	...
	if ($db->db_connected)
	{
		$this->query_result = $db->db_query('SELECT * FROM notes WHERE id=\''.$_GET['note_id'].'\' AND ticket_id=\''.$_GET['id'].'\'');
