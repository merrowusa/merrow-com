...
function slogic_show_announcement()
{
	...
	$output = join ('', file (SCRIPT_PATH.'modules/users/slogic/templates/functions/slogic_show_announcement.tpl'));

	$db = new db_module(DB_HOST,DB_USER,DB_PASS,DB_DATABASE);
	if ($db->db_connected)
	{
		// get announcement
		$this->query_result = $db->db_query('SELECT id,subject,content,sent_date,show_date FROM announcements WHERE (id=\''.$_GET['id'].'\') AND (show_date != \'never\') AND (show_to != \'staff\')');
