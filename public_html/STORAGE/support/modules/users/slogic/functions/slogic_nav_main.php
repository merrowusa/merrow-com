...
{
	// we need the navigation from the index.php file, so make it global
	...
	for ($i=0; $i != sizeof($module); $i++)
	{
		...
		$navigation_template = join ('', file (SCRIPT_PATH.'modules/users/slogic/templates/functions/slogic_nav_main.tpl'));
		...
		{
			$tmp_count = '';
			if (substr($get_navigation[$j], 0, 13) == 'tickets_list_')
			{
				$tmp_function = 'tickets_count_'.substr($get_navigation[$j],13).'_tickets';
				if (function_exists($tmp_function))
				{
					$tmp_count = $tmp_function();
				}
				else 
				{
					require SCRIPT_PATH.'modules/users/tickets/functions/'.$tmp_function.'.php';
					$tmp_count = $tmp_function();
				}
				$tmp_count = ' - '.$tmp_count;
			}
		...
		$make_navigation[$i] = join ('', file (SCRIPT_PATH.'modules/users/slogic/templates/functions/slogic_nav_module.tpl'));
	...
	return '<table><tr><td valign="top">'.implode('</td><td valign="top">',$make_navigation).'</td></tr></table>';
