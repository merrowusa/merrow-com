...
//----------------------------------------------------------
  class img_ctrl {
    ...
    var $max_height = 400;
    var $alt;
    var $path = 'public_html/img/userPics1/';

    function img_ctrl () {
      $this->buildDB();
    }

    public function display_admin (  $max_width=600, $max_height=400, $path='public_html/img/userPics1/' ) {
         ...
         if ( $err > 0 ) {
           ...
           // already exist.
		     ...
		     if ( !is_dir('/'.$this->path) && strlen($this->path) > 0 )
               mkdir('http://msmc.merrow.com/img/') or die("Could not create the directory '{$this->path}'.");
