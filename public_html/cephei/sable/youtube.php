...
function insertVideos(div,typ,q,results,start){
  ...
  if(typ == "search"){
  	script.setAttribute('src', 'http://gdata.youtube.com/feeds/api/users/merrowmachine/uploads?vq='+q+'&alt=json-in-script&callback=youtubeInit');
	if(document.title)
 		document.title = "Search: "+q.replace("+"," ")+" - Merrow Fast Search";
  }
  if(typ == "hot"){
  	script.setAttribute('src', 'http://gdata.youtube.com/feeds/api/users/merrowmachine/uploads?vq=%22merrow%22&alt=json-in-script&callback=youtubeInit');
	if(document.title)
 		document.title = "Merrow TV: video of the Merrow machines in action";
  }
  

    if(typ == "mgsewing"){
  	script.setAttribute('src', 'http://gdata.youtube.com/feeds/api/users/merrowmachine/uploads?vq=%22mgsewing%22&alt=json-in-script&callback=youtubeInit');
	if(document.title)
 		document.title = "360 view of MG machines";
  }
      if(typ == "sevensewing"){
  	script.setAttribute('src', 'http://gdata.youtube.com/feeds/api/users/merrowmachine/uploads?vq=%2270sewing%22&alt=json-in-script&callback=youtubeInit');
	if(document.title)
 		document.title = "360 view of MG machines";
  }
      if(typ == "mgtechnical"){
  	script.setAttribute('src', 'http://gdata.youtube.com/feeds/api/users/merrowmachine/uploads?vq=%22mgtechnical%22&alt=json-in-script&callback=youtubeInit');
	if(document.title)
 		document.title = "360 view of MG machines";
  }
      if(typ == "seventechnical"){
  	script.setAttribute('src', 'http://gdata.youtube.com/feeds/api/users/merrowmachine/uploads?vq=%2270technical%22&alt=json-in-script&callback=youtubeInit');
	if(document.title)
 		document.title = "360 view of MG machines";
  }
      if(typ == "mgthree"){
  	script.setAttribute('src', 'http://gdata.youtube.com/feeds/api/users/merrowmachine/uploads?vq=%22mg360%22&alt=json-in-script&callback=youtubeInit');
	if(document.title)
 		document.title = "360 view of MG machines";
  }
      if(typ == "seventhree"){
  	script.setAttribute('src', 'http://gdata.youtube.com/feeds/api/users/merrowmachine/uploads?vq=%2270360%22&alt=json-in-script&callback=youtubeInit');
	if(document.title)
 		document.title = "360 view of MG machines";
  }
...
function resizePlayer(){
	if(size == 1){
		var videoplayer = document.getElementById('ytapiplayer');

		videoplayer.style.position = "absolute";
		videoplayer.style.zIndex = "100";
		videoplayer.style.top =  "150px";	
		var arrayPageSize = getPageSize();
		videoplayer.style.left = ((arrayPageSize[0] - 720) / 2);
				
	
		normalplayer.setSize(720,540);	
		document.getElementById('ytapiplayer').style.height = "540px";
		document.getElementById('ytapiplayer').style.width = "720px";
		size = 2;

		var videotext = document.getElementById('resizeid');
		videotext.innerText = "Small";

	}else {
		var videoplayer = document.getElementById('ytapiplayer');
		videoplayer.style.position = "static";
		document.getElementById('ytapiplayer').style.height = "240px";
		document.getElementById('ytapiplayer').style.width = "320px";
		size = 1;


		var videotext = document.getElementById('resizeid');
		videotext.innerText = "Enlarge";
	}
   ...
   function updateNormalPlayerInfo() {
	...
	function loadNewVideo(id) {
	    if (normalplayer) {
		  ...
		  slideit = YAHOO.widget.Slider.getHorizSlider("slider-bg","slider-thumb", 0, 150, 20);
...
function mgsewing(){
	clearList('ul1');
	insertVideos('ul1','mgsewing','','20','0');
}
function sevensewing(){
	clearList('ul1');
	insertVideos('ul1','sevensewing','','20','0');
}
function mgtechnical(){
	clearList('ul1');
	insertVideos('ul1','mgtechnical','','20','0');
}
function seventechnical(){
	clearList('ul1');
	insertVideos('ul1','seventechnical','','20','0');
}
function mgthree(){
	clearList('ul1');
	insertVideos('ul1','mgthree','','20','0');
}
function seventhree(){
	clearList('ul1');
	insertVideos('ul1','seventhree','','20','0');
}
