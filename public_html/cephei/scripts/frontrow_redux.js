...
AC.iPhoneFrontRow = Class.create();
Object.extend(Object.extend(AC.iPhoneFrontRow.prototype, Event.Listener), {
	...
	initialize: function(displayPanel, descriptionPanel, sections, options) {
	...
	show: function(section) {
		...
		var showMovie = function(movie) {
			...
			//don't deactivate it if it was the same section we 
			//were reactivating because it wasn't playing
			if (this.currentSection && section != this.currentSection) {
				this.currentSection.deactivate();
			}
 ...
 * display a given section
 */
AC.iPhoneFrontRowSection = Class.create();
Object.extend(Object.extend(AC.iPhoneFrontRowSection.prototype, Event.Publisher), {
	 ...
	 * intiialize section with necessary information
	 */
	initialize: function(trigger, title, movieId, movieUrl, movieOptions, description, options) {
	...
	/**
	 * Sets the trigger to indicate the correct state
	 */
	activate: function() {
		Element.addClassName(this.trigger, 'active');
	},
	...
	getMovie: function() {
		return AC.Quicktime.packageMovie(this.movieId, this.movieUrl, this.movieOptions);
	}
