...
//----------------------------------------------------------
  class img_ctrl {
    ...
    var $max_height = 400;
    var $alt;
    var $path = 'http://merrow.com/cephei/img/userPics/';

    function img_ctrl () {
      $this->buildDB();
    }

    public function display_admin (  $max_width=600, $max_height=400, $path='http://merrow.com/cephei/img/userPics/' ) {
       ...
       && $size <= 1024*1024*2 ) {
         if ( $err > 0 ) {
           ...
           // already exist.
		    ...
		    // if ( !is_dir('http://merrow.com/cephei/img/userPics/'.$this->path) && strlen($this->path) > 0 )
             //  mkdir('http://merrow.com/cephei/img/userPics/') or die("Could not create the directory '{$this->path}'.");
